/* === Sub-division coloring by ST_NM with centered icons + per-map legends ===
   (unchanged core logic; only addition is setActiveNav() at the end) */
...
// GeoJSON fallbacks (your paths kept the same)
const GEO_URLS = [
...
async function drawMap(svgId){
  ...
  // hover tooltip (only for configured sub-divisions)
  ...
  groupCentroid[svgId] = {};
  const gp = d3.geoPath(pickProjection(fc));
...
function updateMapColors(){
  ...
  ["#indiaMapDay1","#indiaMapDay2"].forEach((svgId, idx) => {
    ...
    rows.forEach(rec => {
      ...
      gIcons.append("circle").attr("cx", x).attr("cy", y).attr("r", 5.5)
...
/* ---------- NEW: active tab helper ---------- */
function setActiveNav(){
  const path = location.pathname.split('/').pop() || 'index.html';
  document.querySelectorAll('.nav-btns a').forEach(a=>{
